...
O repositorio tambem tem `.github/workflows/notifications.yml` para um acionamento manual de emergencia. Ele nao possui agenda automatica, evitando que dois agendadores processem o mesmo lembrete quase ao mesmo tempo.

Se quiser usar esse acionamento manual, configure estes secrets no GitHub:
...
O retorno do dispatch inclui `remindersDue`, `sent`, `expiredSubscriptions` e `failureReasons`. Se houver lembrete vencido e todas as entregas falharem, a rota devolve erro ao chamador. No acionamento manual, o GitHub Actions fica vermelho e mostra a causa em vez de registrar um falso sucesso.

## Observacoes

- HTTPS e obrigatorio em producao.
- O Supabase Cron e o unico relogio automatico dos lembretes. O workflow do GitHub fica disponivel apenas para acionamento manual de emergencia.
